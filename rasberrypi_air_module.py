...
import csv
#itertools needed for islice to read specific lines of CSV
...
landed = 0 #Rocket landed -- Controls what is being sent
...
while (flightComplete == 0):
    ...
    with open(calibration, "r"):
        calReadAlt = calibration.readline(1)
        agl = asl - calReadAlt
    
    ltime = datetime.now().strftime('%H-%M')
    dataRow = [ltime, cax, cay, caz, cgx, cgy, cgz, cmx, cmy, cmz, pressure,
               asl, agl]
    ...
    with open(dataCSV, "r"):
        dataRead = csv.reader(dataCSV, delimiter = ',')
        for row in reversed(dataRead):
            lastAlt = row[12]
            if (lastAlt == asl):
                if (landTimerStarted == 0):
                    landStart = time.time()
                else:
                    landCurrent = time.time()
                    landTimeElapsed = landStart - landCurrent
                    if (landTimeElapsed == 30):
                        flightComplete = 1
